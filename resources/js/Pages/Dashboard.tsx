    ...
    return (
        <AppLayout header="Dashboard">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 mb-8">
                <StatCard label="Total Customer"      value={stats.total_customers}        color="bg-blue-500" />
                <StatCard label="Kontrak Aktif"       value={stats.total_active_contracts}  color="bg-emerald-500" />
                <StatCard label="Sales Order (bln)"   value={stats.total_sales_orders}      color="bg-violet-500" />
                <StatCard label="Work Order (bln)"    value={stats.total_service_orders}    color="bg-orange-500" />
                <StatCard label="Pendapatan Bulan ini" value={fmt(stats.revenue_this_month)} color="bg-teal-500" />
                <StatCard label="Invoice Jatuh Tempo" value={stats.overdue_invoices}        color="bg-red-500" />
            </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                ...
                <div className="bg-white rounded-xl shadow p-5">
                    <div className="flex items-center justify-between mb-4">
                        <h3 className="font-semibold text-gray-800">Work Order Aktif</h3>
                        <Link href="/work-orders" className="text-sm text-emerald-600 hover:underline">Lihat semua</Link>
                    </div>
                    <table className="w-full text-sm">
                        ...
                        <tbody>
                            {activeServiceOrders.length === 0 ? (
                                <tr><td colSpan={4} className="py-4 text-center text-gray-400">Tidak ada data</td></tr>
                ...
                <div className="bg-white rounded-xl shadow p-5">
                    <div className="flex items-center justify-between mb-4">
                        <h3 className="font-semibold text-gray-800">Invoice Jatuh Tempo</h3>
                        <Link href="/invoices" className="text-sm text-emerald-600 hover:underline">Lihat semua</Link>
                    </div>
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="text-left text-gray-500 border-b">
