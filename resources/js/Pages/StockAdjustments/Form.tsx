...
const REASONS = [
    'Physical Stock Correction', 'Damaged / Expired Goods', 'Opname Variance',
...
export default function Form({ stockAdjustment, products, warehouses, nextNumber }: any) {
    ...
    return (
        <AppLayout header={editing ? 'Edit Stock Adjustment' : 'Stock Adjustment'}>
            <Head title="Stock Adjustment" />
            <div className="max-w-2xl bg-white rounded-xl shadow p-6">
                <form onSubmit={submit} className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                        <FormField label="Adjustment No." error={errors.adjustment_number} required>
                            <input className={inputCls + ' bg-gray-50'} value={data.adjustment_number} readOnly tabIndex={-1} />
                        </FormField>
                        <FormField label="Adjustment Date" error={errors.adjustment_date} required>
                            <input type="date" className={inputCls} value={data.adjustment_date} onChange={e => setData('adjustment_date', e.target.value)} />
                        </FormField>
                    </div>
                    <FormField label="Product" error={errors.product_id} required>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <FormField label="Adjustment Qty" error={errors.quantity_adjusted} required>
                    ...
                    <div className="flex gap-3 pt-2">
                        ...
                        <Link href="/stock-adjustments" className="px-5 py-2 rounded-md border border-gray-300 text-sm text-gray-700 hover:bg-gray-50">Cancel</Link>
                    </div>
                </form>
            </div>
        </AppLayout>
