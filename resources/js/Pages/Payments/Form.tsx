...
export default function Form({ payment, invoices, bankAccounts, nextNumber }: any) {
    ...
    return (
        <AppLayout header={editing ? 'Edit Payment' : 'Record Payment'}>
            <Head title="Payment" />
            <div className="max-w-2xl bg-white rounded-xl shadow p-6">
                <form onSubmit={submit} className="space-y-4">
                    ...
                    <FormField label="Reference / Transfer No.">
                        <input className={inputCls} value={data.reference} onChange={e => setData('reference', e.target.value)} placeholder="Transfer receipt no..." />
                    </FormField>
                    ...
                    <div className="flex gap-3 pt-2">
                        ...
                        <Link href="/payments" className="px-5 py-2 rounded-md border border-gray-300 text-sm text-gray-700 hover:bg-gray-50">Cancel</Link>
                    </div>
                </form>
            </div>
        </AppLayout>
